...
for imagePath in imagePaths:
    ...
    dirname_list  = imagePath.split(os.path.sep)
    # Label is the number in the directory name, after first "-" 
    label = dirname_list[1].split("-")[1]
...
# Train the network
if DO_AUGMENTATION:
    H = model.fit_generator(aug.flow(train_data, train_labels, batch_size=BS), validation_data=(valid_data, valid_labels), epochs=EPOCHS, verbose=1)
else:
